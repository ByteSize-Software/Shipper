   ...
   #LIVE_URL = 'https://ws.fedex.com:443/xml/'
    ...
    def build_shipment_request(origin, destination, packages, options = {})
      ...
      xml_builder = Nokogiri::XML::Builder.new do |xml|
        xml.ProcessShipmentRequest(xmlns: 'http://fedex.com/ws/ship/v25') do
          build_request_header(xml)
          build_version_node(xml, 'ship', 25, 0 ,0)
    ...
    def build_rate_request(origin, destination, packages, options = {})
      ...
      xml_builder = Nokogiri::XML::Builder.new do |xml|
        xml.RateRequest(xmlns: 'http://fedex.com/ws/rate/v26') do
          build_request_header(xml)
          build_version_node(xml, 'crs', 26, 0 ,0)
          ...
          xml.RequestedShipment do
            ...
            if freight
              ...
              #  xml.HubId(options[:smart_post_hub_id] || 5379) # default to Memphis = FedEx main HQ.
              #end
    ...
    def build_request_header(xml)
      ...
      xml.TransactionDetail do
        xml.CustomerTransactionId(@options[:transaction_id] || 'Shipper1') # TODO: Need to do something better with this...
      end
    end
    ...
    def build_location_node(xml, name, location)
      xml.public_send(name) do
        xml.Address do
          ...
          xml.StreetLines(location.address2) if location.address2
          xml.City(location.city) if location.city
          #xml.StateOrProvinceCode("TN") removes note warning 819 & 820 but needs internationl support first!
          xml.PostalCode(location.postal_code)
          xml.CountryCode(location.country_code(:alpha2))
      ...
      if success
        ...
        if severity == 'ERROR' || severity == 'FAILURE'
          ...
          when *TRANSIENT_TRACK_RESPONSE_CODES
            raise Shipper::ShipmentNotFound, first_notification.at('Message').text
          else
            raise Shipper::ResponseContentError, StandardError.new(first_notification.at('Message').text)
          end
        end
    ...
    def build_document(xml, expected_root_tag)
      ...
      if document.root.nil? || document.root.name != expected_root_tag
        raise Shipper::ResponseContentError.new(StandardError.new('Invalid document'), xml)
      end
      document
    rescue Nokogiri::XML::SyntaxError => e
      raise Shipper::ResponseContentError.new(e, xml)
    end
